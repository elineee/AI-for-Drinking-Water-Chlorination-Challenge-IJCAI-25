...
class CNNModel(AnomalyModel):
    ...
    def get_results(self):
        ...
        for node, contaminated_dfs in all_contaminated_dfs.items():
            ...
            for df in contaminated_dfs[:-1]:
                df_clean, features, labels, predicted_features = self._prepare_data(other_model, df, clean_dfs, node)
                data_train.extend(features)
                data_model_train.extend(predicted_features)
                y_train.extend(labels)
            
            # test data (last dataset)
            df_clean_test, features_test, labels_test,predicted_features_test = self._prepare_data(other_model, contaminated_dfs[-1], clean_dfs, node)
    ...
    def _prepare_data(self, second_model, df, clean_dfs, node):
        ...
        - predicted_features: the features generated by the second model, where each feature is a sliding window of the predicted values of the second model
        """
        ...
        if len(predicted_values) < len(df_clean):
            pad_size = len(df_clean) - len(predicted_values)
        ...
        return df_clean, features, labels,predicted_features
